...
Wave-5 exit (`21368ff`, with `5134488`, `9704551`, `5b21e61`, `a9aba00`,
...
| B9 | Availability characteristics documented (global registry mutex, namespace-clear write lock, O(E) edge cascade); lock split deferred | `e23ebd9` (mitigation doc) |
...
| C5 | i64-compared-via-f64 precision limit documented (value.rs/filter.rs) | `e23ebd9` (docs) |
...
| C12 | MCP convert fidelity limits documented (u64 > i64::MAX → lossy float; non-finite → null) | `e23ebd9` (docs) |
| C13 | Corrupt keymap values skip (no node-0 tombstoning); `Error::CorruptIndex` errors loudly | `19a8a74` |
| C14 | Stop-word-collapsed phrase positions + S-stemmer limits documented | `e23ebd9` (docs) |
| D1 | README iOS/WASM rows made true (CI iOS job; measured wasm size) | `a9aba00` |
| D2 | README true-predicate claim qualified with the `.approx()` opt-out | `e23ebd9` |
| D3 | store.rs/db.rs module docs rewritten to the post-remediation truth | `e23ebd9` |
| D4 | Group keys: bare text, tagged non-text, `t:`-escaped ambiguity | `281e6f3` |
| D5 | DESIGN.md reconciled: layer map, future-spec markers, decision-log dates, closed open-questions, B9/deferred notes | `e23ebd9` |
| D6 | AUDIT.md rewritten as this status doc | `e23ebd9` |
| Verify-candidates batching | Indexed query verification picks its fetch by window density: `candidates × 17 ≥ collection count` takes ONE ordered `for_each` walk of the records (skipping non-candidates by key, stopping past the last), else the historical per-key point-gets; rows, candidate order, filter verdicts, and snapshot scope identical either way (measured crossover ≈5.8% density, dense windows ~22% faster — `selective_window_verify` bench) | `86fbfd2` |
| Sort indexes | A filterless `order_by(field)` with a complete scalar index is served by an index order walk (`PlanShape::SortIndex`): numeric-then-text lane order IS the pinned class-0 total order; same-encoded buckets re-sort with the exact comparator (2^53 i64 collisions), NaN entries skip to the tail, documents fetch only for the `offset+limit` window, and an on-exhaustion scan appends the incomparable/missing tail — identical results by construction, or decline (sources/filters/no-index keep the old paths). `order_by_indexed_5k`: 2.64→0.29 ms asc, 2.79→1.02 ms desc | `1cdb558` |
| Compound prefix-only windows | Re-enabled soundly via per-def `all_docs_indexed` metadata on the compound def record (kind byte; legacy rows decode false): set at backfill completion iff no miss was ever observed — backfill pages commit a miss marker per declined doc, maintenance persists a miss (marker while Building, def-rewrite to false once Complete) in the document's own transaction, and one final completion transaction computes `flag-aware ∧ no marker` (crash windows conservatively false). The planner probe and its `plan_shape` twin admit prefix-only windows only under the flag; any missing/non-encodable-field write permanently clears it until re-registration recomputes (dump/load replays creation, recomputing exactly). `compound_prefix_scan/eq_leading_only_5k`: 1.540 ms → 232.41 µs (−84.9%, 6.6×) | `43c0d8a`, `17e2266` (review fix) |
| Adjacency edge layout | Document-delete edge cascades resolve the deleted key's edges through two private endpoint-first adjacency namespaces (`__adj_out__<coll>`/`__adj_in__<coll>`, one row per edge per endpoint — DERIVED state: the edge rows remain the only source of truth and their format is unchanged), making the cascade O(edges-of-doc) instead of the O(collection-edge-count) two-namespace paged scan. Established inside the first edge write's transaction (fresh databases: an empty build + marker) or the first cascade (legacy pre-adjacency databases), maintained transactionally by `link`/`link_weighted`/`unlink`; a malformed adjacency row falls back to rebuild-from-source-rows + re-run (self-healing). Invisible to `collections()`/`scan`/dump (dump writes source namespaces only; load rebuilds it by the end of load — every replayed edge routes through `link_weighted`, whose `ensure_adjacency_in_txn` establishes it; only a plain reopen defers the re-derive to the first edge write/cascade). `edge_churn/edge_delete_sweep_100`: 241.0 → 40.5 ms (~5.9×, and O(degree) vs O(E) asymptotically); `delete_heavy/delete_half_2p5k`: 566.8 → 194.5 ms (~2.9×) | `a0d37f8` |
| `a__b` migration tooling | `Db::load_with_renames(reader, renames)` maps EVERY collection-name occurrence in a pre-wave-4 dump stream — documents, all index/schema definitions, TTL entries, graph edges, auto-id counters — through the caller's rename table before replay, so index definitions recreate under the new name automatically (they replay via the create-* backfill over the already-renamed records; nothing to re-create by hand). Targets must pass name validation (the offending target's `InvalidName`, upfront); one output name per dump name (`InvalidArgument` for both in-map target collisions and a target colliding with an unmapped dump collection — merging keyspaces would silently overwrite documents); reserved dump names are rejected before mapping (a rename cannot launder an engine-internal namespace); a map entry whose source never occurs is a no-op. The MCP `load` tool takes the same table as an optional `rename` object param. Also closed here: the load auto-id section was the one replay path without reserved-name rejection (B8 family gap; RED-tested) — it rejects now. | `6c3c8a7` |
| >4 GiB dump sections | Dump format v2: the header magic IS the version marker (`CORVIDDUMPv1` legacy / `CORVIDDUMPv2` current, 12 bytes), and every u32 length/count prefix of v1 — byte-field lengths (strings, keys, values), compound/schema per-def field counts, PQ m/k — widens to u64, so no 32-bit representable limit remains anywhere in the format (v1's writer also truncated silently on >4 GiB, `as u32`). Section counts were already u64; fixed-width fields (TTL i64, weight f64, auto-id u64) are byte-identical across versions. Compat: loader accepts v1 AND v2 (prefix width decided by the magic, nothing else differs), writer emits v2 only; an unknown magic (future v3) is `InvalidDump` — the one-way migration story (dump old, load new). Boundary-verified without multi-GiB fixtures: byte-pinned u64 prefixes (values at/past u32::MAX round-trip; prefixes just past 2^32 are honored to EOF, where a u32-narrowing reader would load), a hand-crafted v1 dump loads, and all v1-fixture rename tests stay green. | `9c8f99e`, `97b0eb7` (review fix) |
| In-memory PQ | PQ joins `None`/`Binary`/`Scalar` as an in-memory HNSW storage mode: `create_vector_index_pq(field, metric, m, k)` (and `Hnsw::with_pq` at the direct API), def kind byte `InMemoryPq` (3), and the trained codebook persisted in the index's reserved namespace in the same transaction as the def — so the lazy post-reopen rebuild re-encodes under the SAME codebook, never a retrained one. The metric×storage contract matches the on-disk PQ index exactly: every metric serves (L2 via the ADC table fast path, cosine/dot via reconstruction); ADC for non-L2 stays deferred. Footprint is arithmetic, not an allocator claim: `m` code bytes/vector vs `dim*4` (16× at 64d/m=16, plus the one-off codebook), byte-counted in a unit test. Measured (2000×64d, m=16 k=256): build 367.9 ms vs 124.9 ms None, search 35.8 µs vs 19.1 µs; recall on the fixed clustered conformance corpus measured 1.0 (the public path's over-fetch + exact rerank recover the full top-k; pinned ≥0.7), and the direct-API unit corpus measures 0.56 identical at ef 100/200/400 (pinned ≥0.55 — the thin margin is recorded in docs/BENCHES.md; `ef`-insensitivity is pinned as a contract, the residual gap to exact is codebook resolution, not graph reach). In-memory prune scores by reconstruction rather than rebuilding a k·dim ADC table per prune (identical scores for L2, 2.6× cheaper builds). | `41540aa`, `0274f67` (pins raised) |
| tracing / observability | Structured logging ships behind a non-default `tracing` cargo feature: enabling it adds the `tracing` facade (default features trimmed — `span!`/`event!` only, no `attributes` proc-macro tree; `std` stays on so the feature's own tests can install per-test recorders via thread-local default subscribers; footprint: tracing + tracing-core + pin-project-lite + once_cell) and no public API (all call sites go through the engine-private `telemetry` shim, `src/telemetry.rs`, whose macros expand to nothing when the feature is off — no code, no field evaluation, no dependency). Instrumented at per-operation/per-page granularity: index backfill pages + completion (collection, index family, cursor progress, page size), compaction trigger math + outcome (dead/live counts; in-memory and on-disk), lazy index resumes + adjacency rebuilds (with the absent-vs-stale-marker reason), query plan-shape selection (the arm that drove candidates + its row count; labels are named after the `PlanShape` variants with two deliberate divergences — `indexed_window` carries no family discriminator, the scalar/compound/geo/or kind is `plan_shape()`/`explain()`'s to report, and `stream_scan` is intentionally finer than `PlanShape::Scan`, splitting the bounded streaming filter pass from the materializing fallback — the walk-vs-pointgets decision is visible), the order-index walk's tail-scan fallback, the edge-cascade corrupt-row fallback (rebuild + re-run), and semantic-cache hit/miss. Counters are subsumed by these events (a subscriber counting `plan_shape` per shape is the index-probe counter per shape — families within `indexed_window` split via `plan_shape()`/`explain()`, not the event; `semantic_cache_hit`/`miss` is the cache-hit-rate counter); explicitly still future: plan-cache hit counters (`PlanCache` is host-side — the engine sees no get/miss traffic) and any metrics-export subsystem. Corrupt/unknown state STILL surfaces as typed errors; the events are observability, not control flow. Conformance: a `cfg(all(test, feature = "tracing"))` module asserts the spans/events actually fire for one build and two query shapes, via a hand-rolled recording subscriber (no `tracing-subscriber` dev-dependency). Posture enforced in CI both ways: the check job builds `--no-default-features` and asserts `cargo tree -p corvid --edges normal` contains no `tracing` (and runs clippy/doc/test with the feature on); the wasm job asserts the wasm-target graph never contains it. | `45daa16`, `56bb53e` (telemetry fix) |
| Page-level single-snapshot | `page`/`page_where` execute their entire chunked walk (1024-key `scan_from` steps) inside ONE `store().read()` closure — the same discipline as the query builder's `run()` → `run_with(reader)` split — so a page is one MVCC snapshot: writes committed between two chunk reads of the walk are invisible to its remainder, and the returned rows always match some committed point in time. Chunked reads INSIDE the transaction preserve bounded memory (`ReadBatch::scan_from` pages); it is byte-identical to `Store::scan_from` on a static database (pinned by store.rs's `read_batch_ops_match_store_versions`), so results, `Page{rows,next}` shape, and the cursor contract (`after=b""` skipping only the empty key; a full page always yields a cursor) are unchanged — the queries.rs page pins stayed green untouched. Proof: store.rs's `chunked_read_batch_walk_ignores_mid_walk_writes` commits insert-ahead/delete-ahead/delete-behind/overwrite-ahead writes on the SAME thread between the first and second chunk reads (deterministic — no channels/sleeps) and asserts the concatenated walk equals the opening corpus exactly, while the identical timing against per-chunk-own-transaction walks demonstrably yields a mixed state that never existed as a commit. Snapshot-holding cost (documented on `page_where`): redb is MVCC — the read txn never blocks the single writer, but pins the freed pages of commits landing during the walk (temporary file growth, reclaimable by `Db::compact`); bounded because the walk stops after `limit` rows, and successive page calls each open their own snapshot (keyset-pagination contract). | `f4293b0` |
| Parallel PQ training | PQ k-means training parallelizes deterministically over a scoped std-only worker team (`src/team.rs`, `thread::scope`-spawned per training call, `available_parallelism` capped at 8, no new dependencies and no MSRV change): each Lloyd iteration's assignment step — every training point's nearest centroid, a pure per-point function — is chunk-parallel with item-indexed outputs, while iterations stay sequential (each depends on the last) and the update step consumes assignments in input order, so the trained codebook is bit-identical to the sequential path's (pinned by `pq.rs`'s equivalence test on a corpus large enough to engage the team; all recall/reproducibility corpora unchanged and green). `pq_train_2k_64d`: 177.2 → 67.4 ms (2.6×); `pq_train_10k_128d`: 1465.0 → 356.2 ms (4.1×, the win grows with corpus × k). The HNSW-build half of this row was measured and DECLINED: batching layer-search distance evaluations behind a speculative memo (insertion order, levels, and heap decisions kept sequential) measured 0.21-0.45× at 2k×64d and 0.20-0.25× at 10k×128d — the heap loop consumes distances one at a time, so a multi-microsecond fork/join handshake outweighs the microseconds of pure work it can batch, and 7 spin-polling workers degraded the caller's own sequential phase; pre-computing the per-vector PQ encode/probe in one batch measured ~1.0× (the materialized ADC tables cost more memory traffic than the parallelism saves). `hnsw_build_2k_64d` is byte-for-byte the sequential path and stays 124.1 ms (baseline 123.3); builds keep the seeded-PRNG/insertion-order determinism untouched. Numbers and method: Task 13 report; a SIMD kernel (next row) is the more promising per-eval lever. | `72df3ed`, `0a69155` (fix round) |
| Endpoint-direct `neighbors`/`traverse` reads | `neighbors`/`in_neighbors`/`neighbors_weighted`/`traverse`'s frontier expansion are served from the Task-7 adjacency namespaces through the exact length-delimited `(endpoint, relation)` pair prefix — byte-identical order to the source scans by construction (the same row set under raw-byte order of the same trailing field; traverse's BFS order is derived from that order and unchanged), weights read verbatim from the adjacency values. An empty adjacency scan resolves the built-marker with ONE point-get: current ⇒ genuinely empty, absent ⇒ the source edge-namespace prefix scan answers (legacy pre-adjacency databases, never-linked collections); a traversal hoists the marker and the namespace-name formatting out of its hop loop. The non-empty serve needs no marker check: rows and marker commit in ONE transaction, so rows on a snapshot imply complete-there. MEASURED VERDICT (`neighbors_hub_10k`, hub-heavy + uniform, docs/BENCHES.md): **parity, ±2% on every reader** — the original decline's asymptotics were right (a fixed-`relation` source prefix was already one contiguous range) while its two stated reasons were wrong: no other relation's rows need skipping (the pair prefix is length-delimited and exact) and no weight re-fetch happens (adjacency values carry them). Kept at parity anyway: reads and the delete cascade share one endpoint-keyed layout, results are provably identical, and an endpoint-wide neighbor API (this row's old reopen trigger) is pre-served. | `b850bd2` |
| SIMD distance kernels | MEASURED CLOSURE (ledger-closure Task 2; method + tables in docs/BENCHES.md): LLVM already auto-vectorizes both kernels to 4-wide NEON (`fmul.4s`+`fadd.4s` over `ldp q` pairs, horizontal reduction emitting exactly the source summation order — release-assembly-verified); the hot kernels hold 62–83% of the machine's same-shape read ceiling across dims 64–3072 with no small-dim cliff (64d: 92.7 GB/s = 83% of its 111.9 GB/s ceiling; 768d: 78.6 = 64% of 121.9), and the ceiling is only reachable by REASSOCIATING `f32` sums — both measured faster shapes do exactly that and are declined by the bit-exactness oracle (16 accumulator lanes: −29% at 768d but a different summation order/tree, +14% slower at 64d; fp-fusion via `mul_add`: different rounding AND de-vectorizes to scalar `fmadd`, +24% slower on stable Rust — assembly-checked). Explicit SIMD cannot beat the emitted code without those two levers; `portable_simd` is nightly-only, unsafe intrinsics are out of posture. Volume scans are memory-side: an exact 768d scan sustains 41–42 GB/s (per-eval ≈ the hot kernel's own rate) inside the measured 43–58 GB/s single-core streaming band, so kernel-side wins cannot pass through at corpus scale. The throughput lever that IS available already ships: Binary quantization's packed-byte Hamming scan measures **50.8× faster** than full precision at 768d (`quantized_scan`; Scalar's reconstruct-then-distance pays 1.28× — per-eval decode). No kernel code changed; the numbers support none. | `113f5e5` |
