...
|---|---|
| Endpoint-direct `neighbors`/`traverse` reads | The Task-7 adjacency namespaces serve the delete cascade; `neighbors`/`in_neighbors`/`traverse` keep their `(relation, endpoint)` prefix scans over the edge namespaces — already direct for a FIXED relation (the relation is the leading key component). Adjacency would only win a hypothetical all-relations-of-one-endpoint read, which no public API issues; per-relation reads would additionally have to skip other relations' rows and re-fetch weights (adjacency rows carry them, but the scans stay O(edges-of-(rel,endpoint)) either way). Revisit if an endpoint-wide neighbor API lands. |
