...
fn queries_order_by_indexed_vs_scan_equivalent() {
    ...
    //
    // PINNED ACTUAL BEHAVIOR: order_by alone never consults an index — with
    // no filter there is no window to drive, so both plan as Scan. With a
    // filter that drives the index, the ordering is applied ON TOP of the
    // index-window results and is identical to the scan side.
    let docs: Vec<(&[u8], Value)> = vec![
    ...
    // Unfiltered order_by plans as Scan even with the index present.
    assert_eq!(
    ...
    // A filter on the INDEXED field drives the window.
